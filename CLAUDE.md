...
- `src/classpath.rs` + `src/jdk.rs` + `src/jimage.rs` — `ClassResolver` tries the directory classpath first, then the JDK boot image (`JImage`). JDK classes like `String`/`ArrayList`/`HashMap` are actually interpreted from OpenJDK bytecode, which is why float ops, `Unsafe`, `Class` mirrors, typed `HashMap$Node[]` arrays etc. exist. Both JDK 21 (CI) and JDK 27 must work, so shims target natives and `@IntrinsicCandidate`s, not version-specific private helpers.
...
- `src/vm/` — the interpreter, split into `impl Interpreter` blocks across files (all `pub(super)`):
  ...
  - `natives.rs` — the native-method table. When resolution lands on an `ACC_NATIVE` method, `invoke_native` matches on `(class, name, descriptor)` (`System.arraycopy`, `Object.clone`, `CDS` stubs, `Class` natives, ...). Prefer adding real natives here over intercepting Java methods in `invocation.rs`. `natives/unsafe_memory.rs` serves `Unsafe` array accessors through the byte views in `heap/memory.rs` (base offset 16, little-endian, matching `UnsafeConstants.BIG_ENDIAN = false`).
  - `strings.rs` — the only Rust-side API for `java.lang.String` objects (`java_string`, `new_java_string`, `intern_string`, `is_java_string`, `pop_java_string`). A string is a plain `java/lang/String` instance with the JDK's `value: [B` + `coder: B` fields (Latin-1 or UTF-16LE), so the JDK's `String` bytecode runs on it; `ldc` interns literals through `interned_strings` (a GC root).
  - `resolution.rs` — class loading via resolver, virtual/interface method lookup up the hierarchy, assignability checks.
  - `lifecycle.rs` — GC root collection (`collect_if_needed`), `boot_runtime` (runs `System.setJavaLangAccess()` before `main`, the one piece of `initPhase1` JDK code needs), and `<clinit>` class initialization with re-entrancy guards. Initialization is triggered by `getstatic`/`putstatic` only (not `invokestatic`/`new` as JVMS §5.5 requires); enabling that pulls `Math.<clinit>` (needs `double`) and `SimpleDateFormat.<clinit>` (needs `ConcurrentHashMap`) into every run.
  - `mirrors.rs` — `java.lang.Class` mirrors keyed by runtime type name (`java/lang/String`, `[I`, `int`), with the VM-injected `Class` fields populated so the JDK's `getName`/`isArray`/`getComponentType` bytecode works.
